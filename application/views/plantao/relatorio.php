                ...
                <?php if (($chamado->getIdsetor()) == ($setor->getIdsetor())){ ?>
                    ...
                    <?php if ($chamado->getIdocorrencia_estado() == 1){ //chamados em aberto?>
                        <strong>- Chamado em aberto nº 
                            <a href="<?php echo base_url("ocorrencia/buscar")."/".strval($chamado->getIdocorrencia()); ?>" target="_blank">
                    ...
                    <?php if ($chamado->getIdocorrencia_estado() == 2){ //chamados em atendimento?>
                        ...
                        <br/>
                        <?php $comentarios = $comentario->buscaIdOcorrencia($chamado->getIdocorrencia(), "ASC"); //recebe todos coumentarios ?>
                    ...
                    <?php if ($chamado->getIdocorrencia_estado() == 3){ //chamados em fechado?>
                        ...
                        <?php if (count($comentarios) > 1){ ?>
                            <?php $solucao[0] = array_pop($comentarios); //extrai e recebe ultimo elemento (a solução) e retira a mesma do array ?>
                            <strong>- Aditamentos:</strong>
                            <br/>
                            <div class="descricao">
                                <?php foreach ($comentarios as $value) { ?> 
                        ...
                        <br/>
                        <div class="descricao">
                            <?php echo "<em>".date("d/m/Y", strtotime($solucao[0]->getData()))." ".
                                date("H:i", strtotime($solucao[0]->getData()))."</em>: "; ?>
                            <?php echo str_replace("\r\n", "<br/>", $solucao[0]->getDescricao()); ?>
                        </div>
                        <br/>
                    <?php }?>
                <?php } ?>
